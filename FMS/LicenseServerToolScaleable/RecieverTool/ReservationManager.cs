...
namespace RecieverTool
{
    internal class ReservationManager
    {
        ...
        public void CreateReservationEntry(int RGid, int deviceId, List<AddReservationEntry> featureEntry, Action<string> responseHandler)
        {
            ...
            Task<IRestResponse> currentTask = _client.ExecuteTaskAsync(request);
            if (currentTask.Result.StatusCode == HttpStatusCode.Created)
            {
                responseHandler("CREATED SOME RESERVATION ENTRIES");
            }
            else
            {
                responseHandler($"CREATED RESERVATION FAIL {currentTask.Result.Content}");
            }
        }
